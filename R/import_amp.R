...
import_amp <- function(file, from = "QuantStudio") {
  ...
  if (any(is.na(skip))) {
    stop("The \"Results\" and/or \"Amplification Data\" sheet(s) in the input file do(es) not contain the required \"Well\" column.",
         call. = FALSE)
  }
  ...
  if (from == "QuantStudio") {
    ...
    if (!all(c("Well Position", "Cycle", "Target Name", "Rn", "Delta Rn") %in% colnames(amp_data))) {
      if (all(c("Well", "Target Name", "Rn", paste0("\u0394", "Rn")) %in% colnames(amp_data))) {
        stop("The \"Amplification Data\" sheet in the input file was generated by the StepOne software. Please provide from = \"StepOne\".",
             call. = FALSE)
      } else {
        stop("The \"Amplification Data\" sheet in the input file does not contain one or more required column(s).",
             call. = FALSE)
      }
    }
  ...
  if (from == "StepOne") {
    ...
    if (!all(c("Well", "Cycle", "Target Name", "Rn", paste0("\u0394", "Rn")) %in% colnames(amp_data))) {
      if (all(c("Well Position", "Cycle", "Target Name", "Rn", "Delta Rn") %in% colnames(amp_data))) {
        stop("The \"Amplification Data\" sheet in the input file was generated by the QuantStudio software. Please provide from = \"QuantStudio\".",
             call. = FALSE)
      } else {
        stop("The \"Amplification Data\" sheet in the input file does not contain one or more required column(s).",
             call. = FALSE)
    ...
    if (!all(c("Well", "Sample Name", "Target Name") %in% colnames(sample_data))) {
      stop("The \"Results\" sheet in the input file does not contain one or more required column(s).",
           call. = FALSE)
    }
